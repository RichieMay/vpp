			       ...
			       char *name)
{
  ...
  if (base == 0)
    ...
    {
      /* 原无锁遍历竞态已收敛到持 map_lock 的 clib_mem_vm_lookup_log2_page_sz
       * （见 linux/mem.c 注释）。churn 下并发 map/unmap 会使无锁遍历走到被释放的
       * mprotect 页而 SIGSEGV。*/
      log2_page_sz = clib_mem_vm_lookup_log2_page_sz (base);
    }
