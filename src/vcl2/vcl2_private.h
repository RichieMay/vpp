...
typedef struct
{
  vcl2_handle_t handle;		/* app 侧 correlation key（= connect/listen 的 context） */
  ...
  uint8_t in_use;
  uint8_t nonblocking;		/* fcntl(F_SETFL O_NONBLOCK)/ioctl(FIONBIO) 设置：recv/send 无数据/空间时返回 EAGAIN */
  ...
  uint8_t rmt_ip[16];
  uint16_t rmt_port;
  vcl2_handle_t *accept_q;	/* 已 ACCEPTED、待 accept() 取走的新 session handle 队列 */
  uint32_t accept_context;	/* 子 session 的 ACCEPTED.context，回 ACCEPTED_REPLY 用 */
} vcl2_session_t;
...
typedef struct
{
  ...
  svm_msg_q_t *vpp_evt_q;	/* 每 VPP-worker 的 RX mq：send 后通知 VPP 取 tx_fifo（CONNECTED 回填） */
...
#define VCL2_VPP_WRK_SEG_HANDLE(wrk) (VCL2_INVALID_SEGMENT_HANDLE - (u64)(wrk) - 1)
...
int vcl2_sapi_recv_fd (int *fd);	/* recvmsg 一个 SCM_RIGHTS fd（ADD_SEGMENT 用）*/
void vcl2_atfork_child (void);		/* fork 子进程：重建 worker 身份（nginx 多 worker）*/
void vcl2_atfork_parent (void);		/* fork 父进程（master）：从 accept 转发摘除自己 */
int vcl2_session_unlisten (vcl2_handle_t h); /* 把本 worker 移出 listener 的 accept 轮转 */
...
int vcl2_segment_attach_mq (u64 handle, uword offset, u32 idx, svm_msg_q_t **mq);
